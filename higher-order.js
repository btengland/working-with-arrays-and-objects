...
let orderTotals = orders.map((elem) => {
  return (elem.tax * elem.price) + elem.price
})
...
let bobsTotal = purchases.reduce((total, purchased) => {
  if (purchased.owner === 'Bob'){
    return total + purchased.price 
  } else {
    return total
  }
} ,0
